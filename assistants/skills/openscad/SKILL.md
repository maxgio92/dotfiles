...
Resolve each skill-root placeholder from the installed path of that skill's `SKILL.md`.
...
```bash
<openscad-skill-root>/scripts/version-scad.sh <name>
```
...
```bash
<preview-skill-root>/scripts/render-scad.sh <name>_<version>.scad --output <name>_<version>.png
```
...
1. Check for existing versions:
   ```bash
   <openscad-skill-root>/scripts/version-scad.sh piano
   ```
...
3. Render preview:
   ```bash
   <preview-skill-root>/scripts/render-scad.sh piano_001.scad --output piano_001.png
   ```
