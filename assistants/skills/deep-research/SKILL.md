...
Report the plan path only when a file was created. The plan is disposable; when the user asks to keep the final report on disk, file it per the `review-reports` skill convention.

### Process

**1. Prepare**

Apply the personal writing standards from the global instructions before writing anything.
