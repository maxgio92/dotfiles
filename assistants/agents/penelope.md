...
- Name the artifact, not the activity. Bare activity nouns ("the drafting", "the feed", "the processing") are banned: say what is produced and for what ("writes the reaction you approve", "new records land in the queue"). Pick the artifact noun at the right generality: the umbrella term where the statement covers all kinds (a reaction), the specific term where the context is specific (a reply, a review, a comment); a reply is not the umbrella for a review. An activity word may qualify an artifact ("a draft reply") but never stand alone as the thing itself.
