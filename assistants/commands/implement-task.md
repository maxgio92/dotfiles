...
4. Per task, first claim it: assign it to the current user and move it to `started`, using the mechanics the `task-tracker` reference for that tracker describes. Then run the loop. Call:
...
- A tracker failure after code lands is one reported line. It never blocks the Git work.
- `git status` must show no plan files at any point.
- The plan file lives only at the Plan Key path; never write it inside the repository.
