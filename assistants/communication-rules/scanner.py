...
from core.dispatch import AGENTS, dispatch, shape_response

# Command-hook agents whose runtime consumes the agent's native wire JSON on
# stdout. They emit nothing for a pass. Plugin agents also consume a shaped
# response, but their in-process shim applies that data to live runtime objects.
    ...
    if args.mode in AGENTS:
        ...
        if args.mode in COMMAND_HOOK_AGENTS:
            ...
            if shaped is not None:
                ...
                sys.stdout.write("\n")
            return 0
        # Pi and OpenCode load an in-process shim that applies this flat plugin
        # response to live runtime objects.
        shaped = shape_response(args.mode, args.event, decision, config)
        sys.stdout.write(json.dumps(shaped))
        sys.stdout.write("\n")
        return 0
