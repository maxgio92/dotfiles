...
- Command agents emit runtime-specific JSON on stdout.
...
#
# Claude Code and Codex share the deny and Tier A facing wire shapes. Claude
# Code accepts an explicit allow. Codex accepts permissionDecision "allow" only
# when a hook also replaces the tool input, so its non-blocking path adds context
# and leaves the permission decision to Codex.
...
#
# Codex permits deny for blocking a tool call. A non-blocking PreToolUse result
# must not return permissionDecision "allow" unless it also returns updatedInput.
# This scanner never rewrites tool input, so yield and allow-revise use
# additionalContext and let Codex apply its normal approval policy.
...
def codex_allow(decision: Decision) -> dict:
    """Add guidance without overriding Codex's tool permission decision."""
    return codex_context("PreToolUse", decision.notice)
...
def codex_response(
    ...
    if decision.decision == "allow-revise":
        # B1 allow-and-correct: Codex receives the revision prompt as context
        # while its normal permission policy controls whether the write lands.
        return codex_allow(decision)
    if decision.decision == "re-issue":
